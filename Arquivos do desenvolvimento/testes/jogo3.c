...
int main(void)
{
    ...
    ALLEGRO_BITMAP *botao_sair = NULL, *area_central = 0, *imagem = NULL;
    ...
    // Carrega a imagem
    imagem = al_load_bitmap("Tela_Inicial1.png");
    ...
    al_set_window_title(janela, "Aprendendo a Pré-Historia");
    ...
    botao_sair = al_load_bitmap("BT_Sair.bmp");
    if (!botao_sair)
    {
        ...
        while (!al_is_event_queue_empty(fila_eventos))
        {
            ...
            else if (evento.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP)
            {
                if (evento.mouse.x >= LARGURA_TELA - al_get_bitmap_width(botao_sair) - 35 &&
                    evento.mouse.x <= LARGURA_TELA - 40 && evento.mouse.y <= ALTURA_TELA - 35 &&
